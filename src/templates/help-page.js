        ...
        <main className="flex-1 relative z-0 overflow-y-auto focus:outline-none">
          <div className="py-1">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
              ...
              <div className="py-4">
                <Article page={props.data.helpPage}/>
              </div>
              {/* /End replace */}
            </div>
          </div>
...
query HelpIndexTmplQuery(
    $slug: String!
    $previousPageSlug: String
    $nextPageSlug: String
  ) {
    allHelpPages: allContentfulPage(sort: { fields: [publishDate], order: DESC }) {
      nodes {
        name
        title
        slug
  ...
  helpPage: contentfulPage(slug: { eq: $slug }) {
      slug
      name
      title
      publishDate(formatString: "MMMM Do, YYYY")
